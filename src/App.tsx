...
// Step indicator — defined outside AppContent so it's stable across renders
...
const AppContent: React.FC = () => {
  ...
  const [monteCarloResult, setMonteCarloResult] = useState<MonteCarloResult | null>(null); // eslint-disable-line @typescript-eslint/no-unused-vars
  ...
  React.useEffect(() => { if (inputs.useRiskHeatmap) calculate(); }, [inputs.useRiskHeatmap, calculate]);
  ...
  const metrics = result
    ? calcSnapshotMetrics(snapshot, result, inputs.accountType, inputs.withdrawal, annualReturn)
    : null;
  ...
  return (
    <>
      {/* ── Mobile (< lg) ── */}
      <div className="lg:hidden min-h-screen bg-slate-100">
        {/* Sticky header */}
        ...
        <div className="max-w-lg mx-auto pt-2 pb-24">
          ...
          {/* Step 2 — Dashboard + full analysis */}
          {currentStep === 2 && (
      ...
      {/* ── Desktop (≥ lg) ── */}
      <div className="hidden lg:flex flex-col min-h-screen bg-slate-100">
        {/* Tab bar */}
        <div className="bg-white border-b border-slate-200 px-6 sticky top-0 z-10">
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-1">
              ...
              {([
                { key: 'results' as DesktopTab, label: `${t.stepResult} & ${t.stepGoal}` },
                { key: 'montecarlo' as DesktopTab, label: t.monteCarloSimulation },
              ]).map(({ key, label }) => (
                <button
                  key={key}
          ...
          <div className="flex flex-1 max-w-7xl mx-auto w-full">
            ...
            <div className="flex-1 bg-slate-50 overflow-y-auto">
              <div className="p-6 space-y-5 max-w-3xl">
                  ...
                  <div className="bg-white rounded-[20px] shadow-[0_4px_20px_rgba(15,23,42,0.07)] overflow-hidden">
                    <div className="flex items-center gap-4 px-6 py-5">
                      <div className="flex-shrink-0">
                        {(() => {
                          const sz = 104, r = sz / 2 - 9, cx = sz / 2, cy = sz / 2;
                            ...
                            <svg width={sz} height={sz} viewBox={`0 0 ${sz} ${sz}`}>
                              <g transform={`rotate(-90, ${cx}, ${cy})`}>
                                ...
                                <circle cx={cx} cy={cy} r={r} fill="none"
                                  stroke={pct >= 1 ? '#16a34a' : '#2563eb'} strokeWidth={9}
                          ...
                          <p className={`text-[20px] font-black ${metrics.gapToTarget > 0 ? 'text-amber-600' : 'text-green-600'}`}>
                            {metrics.gapToTarget > 0
                              ? `−${new Intl.NumberFormat(language === 'zh-TW' ? 'zh-TW' : 'en-US', {
                                  style: 'currency', currency: language === 'zh-TW' ? 'TWD' : 'USD',
                    ...
                    <div className="grid grid-cols-3 gap-3 px-6 pb-5">
                      {[
                        {
                          label: language === 'zh-TW' ? '所需起始資產' : 'Required Assets',
                          value: metrics.targetRequired,
                      ...
                      ].map(({ label, value, className, textClass }) => (
                        <div key={label} className={`rounded-[12px] border px-3 py-2.5 ${className}`}>
                          <p className="text-[10px] font-semibold text-slate-500 mb-1">{label}</p>
                          <p className={`text-[14px] font-black ${textClass}`}>
                            {new Intl.NumberFormat(language === 'zh-TW' ? 'zh-TW' : 'en-US', {
                              style: 'currency', currency: language === 'zh-TW' ? 'TWD' : 'USD',
                      ...
                      <div className={`rounded-[12px] border px-3 py-2.5
                        ...
                        <p className="text-[10px] font-semibold text-slate-500 mb-1">{t.withdrawalSafety}</p>
                        <p className={`text-[14px] font-black
                          ${metrics.currentWithdrawalRate <= 0.04 ? 'text-green-700'
                            : metrics.currentWithdrawalRate <= 0.05 ? 'text-amber-700'
                            : 'text-red-700'}`}>
                          {metrics.currentWithdrawalRate <= 0.04 ? t.safetyLow
                            : metrics.currentWithdrawalRate <= 0.05 ? t.safetyOk
                            : t.safetyHigh}
                ...
                {result && (
                  <Results
                    result={result} error={null} isCalculating={isCalculating}
                    inputs={inputs} onInputChange={handleInputChange} onCalculate={calculate}
                  />
                )}
              </div>
            </div>
